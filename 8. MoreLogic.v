...
Proof.
  unfold not.
  intros.  specialize (H (P x)). 
  inversion H. apply H1. 
  contradiction H0. exists x. apply H1.
Qed.
(** [] *)
...
Proof.
   intros. split. intros. destruct H. destruct H.
...
Inductive all {X : Type} (P : X -> Prop) : list X -> Prop :=
  | all_nil : all P []
  | all_x : forall x l, P x -> all P l -> all P (x :: l)
.
...
Lemma and_true_true : forall (p q: bool),
  andb p q = true -> (p = true) /\ (q = true).
Proof.
  intros. destruct p. split. reflexivity.
...
Proof.
  intros. split. 
  intros. induction l.
  apply all_nil. apply all_x. simpl in H. destruct (P a).
  reflexivity. inversion H.
  apply IHl. simpl in H. apply and_true_true in H.
  destruct H. apply H0.
  intros. induction l.
  simpl. reflexivity.
  inversion H. subst. apply IHl in H3. 
  simpl. rewrite H2. rewrite H3. simpl. reflexivity.
Qed.
(** [] *)
...
Proof.
  ...
  induction xs.
    ...
    destruct ys eqn:Heqe.
      rewrite app_nil in H. left. apply H.
      left. apply ai_here. 
    destruct ys eqn:Heqe.
      left. rewrite app_nil in H. apply H.
      specialize (IHxs (x0 :: l0)).
...
Proof.
  ...
  (*xs=a:: xs*)
  intros. destruct ys eqn:Heqy.
  rewrite app_nil. inversion H. apply H0. inversion H0.
  inversion H. inversion H0. subst. apply ai_here. subst.
  specialize (IHxs (x0 :: l)).
  simpl. apply ai_later. apply IHxs. left. apply H2.
  subst. simpl. apply ai_later. 
  specialize (IHxs (x0 :: l)).  apply IHxs.  right.  apply H0. 
Qed.
...
Inductive nostutter:  list nat -> Prop :=
  | ns_n : nostutter nil
  | ns_x : forall a, nostutter [a]
  | ns_xx : forall a b l, nostutter (b :: l) -> beq_nat a b = false -> nostutter (a :: b :: l)
.
...
Proof. 
  ...
  intros. simpl. apply f_equal. apply IHl1. 
Qed.
...
Proof.
  ...
  destruct H0.
  rewrite H0. exists (b :: witness). exists witness0.
  simpl. reflexivity.
Qed.
