...
Proof.
  ...
  inversion H0. subst. 
    ...
    subst. assumption.
  apply E_IfFalse. 
    unfold bequiv in H; simpl in H. rewrite H. reflexivity.
    assumption.
Qed.
(** [] *)
...
Proof.
  ...
  inversion H; subst.
    (*b=true*)
    apply E_IfFalse. simpl. rewrite H5. simpl. reflexivity. assumption.
    (*b=false*)
    apply E_IfTrue. simpl. rewrite H5. simpl. reflexivity. assumption.
  (*Case <-*)
  inversion H; subst.
    (*BNot b = true*)
    apply E_IfFalse. simpl in H5. apply negb_true_iff. assumption. assumption.
    (*BNot b = false*)
    apply E_IfTrue. simpl in H5. apply negb_false_iff. assumption. assumption.
Qed.
(*[]*)
(** *** *)
...
Proof. 
  ...
  assert (e: ~( (WHILE b DO c END) / st || st' )).
  apply WHILE_true_nonterm. assumption. contradiction.
  assert (e: ~( (WHILE BTrue DO SKIP END) / st || st' )).
...
Proof.
  ...
  Case "->".
    ...
    assert (st' = (update st' X0 (st' X0))).
          apply functional_extensionality. intro. 
          rewrite update_same; reflexivity.
    rewrite H1 at 2. constructor. 
    unfold aequiv in H. rewrite <- H. simpl. reflexivity.
  Case "<-".
    inversion H0; subst. 
